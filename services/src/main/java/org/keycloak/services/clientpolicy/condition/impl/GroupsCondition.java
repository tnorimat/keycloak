...
public class GroupsCondition implements ClientPolicyCondition {

    private static final Logger logger = Logger.getLogger(GroupsCondition.class);

    private final KeycloakSession session;
    private final ComponentModel componentModel;

    public GroupsCondition(KeycloakSession session, ComponentModel componentModel) {
    ...
    private boolean isGoupsMatched(AdminAuth admin) {
        if (admin.getUser() == null) return false;

        admin.getUser().getGroups().stream().forEach(i -> ClientPolicyLogger.log(logger, " user group = " + i.getName()));
        componentModel.getConfig().get(GroupsConditionFactory.GROUPS).stream().forEach(i -> ClientPolicyLogger.log(logger, "groups expected = " + i));

        boolean isMatched = componentModel.getConfig().get(GroupsConditionFactory.GROUPS).stream().anyMatch(i->{
            return admin.getUser().getGroups().stream().anyMatch(j->j.getName().equals(i));
            });
        if (isMatched) {
