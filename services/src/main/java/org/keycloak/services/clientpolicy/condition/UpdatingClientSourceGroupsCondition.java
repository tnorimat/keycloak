...
public class UpdatingClientSourceGroupsCondition implements ClientPolicyConditionProvider {
    ...
    public UpdatingClientSourceGroupsCondition(KeycloakSession session, ComponentModel componentModel) {
    ...
    private boolean isGroupsMatched(UserModel user) {
        ...
        user.getGroups().stream().forEach(i -> ClientPolicyLogger.log(logger, " user group = " + i.getName()));
        componentModel.getConfig().get(UpdatingClientSourceGroupsConditionFactory.GROUPS).stream().forEach(i -> ClientPolicyLogger.log(logger, "groups expected = " + i));

        boolean isMatched = componentModel.getConfig().get(UpdatingClientSourceGroupsConditionFactory.GROUPS).stream().anyMatch(i->{
            return user.getGroups().stream().anyMatch(j->j.getName().equals(i));
            });
        if (isMatched) {
