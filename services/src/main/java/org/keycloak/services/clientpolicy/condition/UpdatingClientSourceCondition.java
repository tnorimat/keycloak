...
public class UpdatingClientSourceCondition implements ClientPolicyConditionProvider {

    private static final Logger logger = Logger.getLogger(UpdatingClientSourceCondition.class);

    private final KeycloakSession session;
    private final ComponentModel componentModel;

    public UpdatingClientSourceCondition(KeycloakSession session, ComponentModel componentModel) {
    ...
    private boolean isAuthMethodMatched(ClientUpdateContext context) {
        ...
        if (context.getToken() == null) {
            authMethod = UpdatingClientSourceConditionFactory.BY_ANONYMOUS;
        } else if (isInitialAccessToken(context.getToken())) {
            authMethod = UpdatingClientSourceConditionFactory.BY_INITIAL_ACCESS_TOKEN;
        } else if (isRegistrationAccessToken(context.getToken())) {
            authMethod = UpdatingClientSourceConditionFactory.BY_REGISTRATION_ACCESS_TOKEN;
        } else if (isBearerToken(context.getToken())) {
            if (context.getAuthenticatedUser() != null || context.getAuthenticatedClient() != null) {
                authMethod = UpdatingClientSourceConditionFactory.BY_AUTHENTICATED_USER;
            } else {
                authMethod = UpdatingClientSourceConditionFactory.BY_ANONYMOUS;
            }
        }
