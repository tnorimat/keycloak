    ...
    static protected void addCommonConfigProperties(List<ProviderConfigProperty> configProperties) {
        ...
        property = new ProviderConfigProperty(
                ...
                "Only effective if Resource Indicators feature (RFC 8707) is enabled. " +
                        "If a resource parameter value of an authorization request is not included in this allow list, the executor rejects the request with invalid_target error. " +
                        "If the request does not include a resource parameter, the executor does not enforce this allow list. " +
                        "The executor keeps an audience mapper on the client synchronized with this allow list: " +
                        "it is added to the client when newly allow-listed, and removed from the client when no longer allow-listed. " +
